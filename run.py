...
class EWCLoRAModel(torch.nn.Module):
    def __init__(self, model_name_or_path, fisher_matrix_path, accelerator, ewc_lambda=1):
        super().__init__()
        self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, load_in_4bit=True, device_map={"": accelerator.local_process_index})
        # self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map={"": accelerator.local_process_index})
        ...
        self.fisher_matrix = AutoModelForCausalLM.from_pretrained(fisher_matrix_path, device_map={"": accelerator.local_process_index})
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            with accelerator.accumulate(model):
                print(model)
                loss, ce_loss, ewc_loss= model(**batch)
                print(loss)
                # loss = outputs.loss
                # import pdb;pdb.set_trace()
